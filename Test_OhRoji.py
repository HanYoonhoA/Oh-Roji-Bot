                ...
                async def send_to_all():
                    for cid in list(active_channels):
                        ...
                        try:
                            ...
                            print(f"⚠️ Could not send to {cid}: {e}")

                asyncio.run_coroutine_threadsafe(send_to_all(), bot.loop)
        except Exception as e:
            print(f"⚠️ Console input error: {e}")
...
from Question_Banks.Basic_Eletrical import week1_questions as week1_ACDC
from Question_Banks.Basic_Eletrical import week2_questions as week2_ACDC
from Question_Banks.Basic_Eletrical import week3_questions as week3_ACDC
from Question_Banks.Basic_Eletrical import week4_questions as week4_ACDC
from Question_Banks.Basic_Eletrical import week5_questions as week5_ACDC
from Question_Banks.Basic_Eletrical import week6_questions as week6_ACDC

from Question_Banks.Basic_Tools import week1_questions as week1_TOOLS
from Question_Banks.Basic_Tools import week2_questions as week2_TOOLS
from Question_Banks.Basic_Tools import week3_questions as week3_TOOLS
# from Question_Banks.Basic_Tools import week4_questions as week4_TOOLS
# from Question_Banks.Basic_Tools import week5_questions as week5_TOOLS
# from Question_Banks.Basic_Tools import week6_questions as week6_TOOLS

from Question_Banks.AME_Standard_Practice import week1_questions as week1_AME
from Question_Banks.AME_Standard_Practice import week2_questions as week2_AME
from Question_Banks.AME_Standard_Practice import week3_questions as week3_AME
# from Question_Banks.AME_Standard_Practice import week4_questions as week4_AME
# from Question_Banks.AME_Standard_Practice import week5_questions as week5_AME
# from Question_Banks.AME_Standard_Practice import week6_questions as week6_AME

from Question_Banks.Aircraft_Pub import week1_questions as week1_Pubs
from Question_Banks.Aircraft_Pub import week2_questions as week2_Pubs
from Question_Banks.Aircraft_Pub import week3_questions as week3_Pubs
# from Question_Banks.Aircraft_Pub import week4_questions as week4_Pubs
# from Question_Banks.Aircraft_Pub import week5_questions as week5_Pubs
# from Question_Banks.Aircraft_Pub import week6_questions as week6_Pubs
...
review_links = {
    ...
    "Metallurgy": {
        ...
        "All Week": "https://docs.google.com/document/d/1RMR6fAC5s3kXTM6p1VcbyxHqCA3zLfCsVugpNlOV5NY/edit?usp=sharing",
    },
    "Intro to CARs": {
...
class_sessions = {
    ...
    "Basic Electrical": 0,
    "Basic Tools": 0,
    "Aircraft Publications": 0,
    "AME Standard Practice": 0
}
...
# 🔄 User mode (practice/exam). Default = practice.
# values: "practice" or "exam"
user_modes = defaultdict(lambda: "practice")

def is_exam_mode(user_id: int) -> bool:
    return user_modes.get(user_id, "practice") == "exam"

def mode_badge(user_id: int) -> str:
    return "🧪 **Exam Mode**" if is_exam_mode(user_id) else "🟩 **Practice Mode**"
...
async def start(interaction: discord.Interaction):
    ...
    current = "Exam" if is_exam_mode(uid) else "Practice"
    ...
    embed = discord.Embed(
        ...
        description=(
            ...
            f"Current answering mode: **{current} Mode**\n"
            f"• Practice: see correct/incorrect after each question\n"
            f"• Exam: results shown **at the end only**"
        ),
    ...
    # Mode toggle row
    view.add_item(ModeToggleButton("Practice Mode", "practice"))
    view.add_item(ModeToggleButton("Exam Mode", "exam"))
...
class ModeToggleButton(discord.ui.Button):
    def __init__(self, label, mode_value):
        style = discord.ButtonStyle.success if mode_value == "practice" else discord.ButtonStyle.danger
        super().__init__(label=label, style=style)
        self.mode_value = mode_value

    async def callback(self, interaction: discord.Interaction):
        uid = interaction.user.id
        user_modes[uid] = self.mode_value

        msg = (
            "🟩 Practice Mode activated — you’ll see **correct/incorrect** after each question."
            if self.mode_value == "practice"
            else "🎯 Exam Mode activated — **results will show at the end only**."
        )
        await interaction.response.send_message(msg, ephemeral=True)

        # Also refresh the original /start panel (optional)
        try:
...
class StudyModeButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if self.mode == "midterm":
            ...
            view.add_item(ClassButton("Basic Electrical", "midterm_acdc"))
            view.add_item(ClassButton("Basic Tools", "midterm_TOOLS"))
            view.add_item(ClassButton("Aircraft Publications","Aircraft_Pub"))
            view.add_item(ClassButton("AME Standard Practice","AME_Standard_Practice"))
            ...
            view.add_item(ClassButton("Basic Electrical", "quiz_acdc"))
            view.add_item(ClassButton("Basic Tools", "quiz_TOOLS"))
            view.add_item(ClassButton("Aircraft Publications","Aircraft_Pub"))
            view.add_item(ClassButton("AME Standard Practice","AME_Standard_Practice"))            
            ...
            view.add_item(ClassButton("Basic Electrical", "midterm_acdc"))
            view.add_item(ClassButton("Basic Tools", "final_TOOLS"))
            view.add_item(ClassButton("Aircraft Publications","Aircraft_Pub"))
            view.add_item(ClassButton("AME Standard Practice","AME_Standard_Practice"))            
...
class ClassButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        elif self.mode == "midterm_acdc":
            qs = week1_ACDC[:10] + week2_ACDC[:10] + week3_ACDC[:10] + week4_ACDC[:10] + week5_ACDC[:10] + week6_ACDC[:10] 
            
        elif self.mode == "midterm_TOOLS":
            qs = week1_TOOLS[:10] + week2_TOOLS[:10] + week3_TOOLS[:10] # (Example: adjust slices as needed)
            
        elif self.mode == "Aircraft_Pub":
            qs = week1_TOOLS[:10] + week2_TOOLS[:10] + week3_TOOLS[:10]
            
        elif self.mode == "AME_Standard_Practice":
            qs = week1_TOOLS[:10] + week2_TOOLS[:10] + week3_TOOLS[:10]
        ...
        elif self.mode == "quiz_acdc":
            view = discord.ui.View(timeout=None)
            view.add_item(WeekButton("Week 1", week1_ACDC, "Basic Electrical"))
            view.add_item(WeekButton("Week 2", week2_ACDC, "Basic Electrical"))
            view.add_item(WeekButton("Week 3", week3_ACDC, "Basic Electrical"))
            view.add_item(WeekButton("Week 4", week4_ACDC, "Basic Electrical"))
            view.add_item(WeekButton("Week 5", week5_ACDC, "Basic Electrical"))
            view.add_item(WeekButton("Week 6", week6_ACDC, "Basic Electrical"))
            return await interaction.response.edit_message(
                embed=discord.Embed(
                    title="🧲 Quiz (5%) — Basic Electrical",
                    description="Pick a week (5 random questions)",
                    color=discord.Color.blue()
                ),
                view=view
        ...
        elif self.mode == "quiz_TOOLS":
            view = discord.ui.View(timeout=None) 
            view.add_item(WeekButton("Week 1", week1_TOOLS, "Basic Tools"))
            view.add_item(WeekButton("Week 2", week2_TOOLS, "Basic Tools"))
            view.add_item(WeekButton("Week 3", week3_TOOLS, "Basic Tools"))
            view.add_item(WeekButton("Week 4", week4_TOOLS, "Basic Tools"))
            view.add_item(WeekButton("Week 5", week5_TOOLS, "Basic Tools"))
            view.add_item(WeekButton("Week 6", week5_TOOLS, "Basic Tools"))
            # (Add all your weeks)
            return await interaction.response.edit_message(
                embed=discord.Embed(
                    title="📝 Quiz (5%) — Basic Tools",
                    description="Pick a week (5 random questions)",
                    color=discord.Color.blue()
                ),
                view=view
        ...
        elif self.mode == "quiz_Pub":
            view = discord.ui.View(timeout=None) 
            view.add_item(WeekButton("Week 1", week1_Pubs, "Aircraft Publications"))
            view.add_item(WeekButton("Week 2", week2_Pubs, "Aircraft Publications"))
            view.add_item(WeekButton("Week 3", week3_Pubs, "Aircraft Publications"))
            view.add_item(WeekButton("Week 4", week4_Pubs, "Aircraft Publications"))
            view.add_item(WeekButton("Week 5", week5_Pubs, "Aircraft Publications"))
            view.add_item(WeekButton("Week 6", week5_Pubs, "Aircraft Publications"))
            # (Add all your weeks)
            return await interaction.response.edit_message(
                embed=discord.Embed(
                    title="📝 Quiz (5%) — Aircraft Publications",
                    description="Pick a week (5 random questions)",
                    color=discord.Color.blue()
                ),
                view=view
        ...
        elif self.mode == "quiz_AME":
            view = discord.ui.View(timeout=None) 
            view.add_item(WeekButton("Week 1", week1_AME, "AME Standard Practice"))
            view.add_item(WeekButton("Week 2", week2_AME, "AME Standard Practice"))
            view.add_item(WeekButton("Week 3", week3_AME, "AME Standard Practice"))
            view.add_item(WeekButton("Week 4", week4_AME, "AME Standard Practice"))
            view.add_item(WeekButton("Week 5", week5_AME, "AME Standard Practice"))
            view.add_item(WeekButton("Week 6", week5_AME, "AME Standard Practice"))
            # (Add all your weeks)
            return await interaction.response.edit_message(
                embed=discord.Embed(
                    title="📝 Quiz (5%) — AME Standard Practice",
                    description="Pick a week (5 random questions)",
                    color=discord.Color.blue()
                ),
                view=view
        ...
        elif self.mode == "final_TOOLS":
            qs = []
            qs += random.sample(week1_TOOLS, min(10, len(week1_TOOLS)))
            qs += random.sample(week2_TOOLS, min(10, len(week2_TOOLS)))
            qs += random.sample(week3_TOOLS, min(10, len(week3_TOOLS)))
            qs += random.sample(week4_TOOLS, min(10, len(week4_TOOLS)))
            qs += random.sample(week5_TOOLS, min(10, len(week5_TOOLS)))
            qs += random.sample(week6_TOOLS, min(10, len(week6_TOOLS)))
            
        elif self.mode == "final_Pub":
            qs = []
            qs += random.sample(week1_Pubs, min(10, len(week1_Pubs)))
            qs += random.sample(week2_Pubs, min(10, len(week2_Pubs)))
            qs += random.sample(week3_Pubs, min(10, len(week3_Pubs)))
            qs += random.sample(week4_Pubs, min(10, len(week4_Pubs)))
            qs += random.sample(week5_Pubs, min(10, len(week5_Pubs)))
            qs += random.sample(week6_Pubs, min(10, len(week6_Pubs)))
            
        elif self.mode == "final_AME":
            qs = []
            qs += random.sample(week1_AME, min(10, len(week1_AME)))
            qs += random.sample(week2_AME, min(10, len(week2_AME)))
            qs += random.sample(week3_AME, min(10, len(week3_AME)))
            qs += random.sample(week4_AME, min(10, len(week4_AME)))
            qs += random.sample(week5_AME, min(10, len(week5_AME)))
            qs += random.sample(week6_AME, min(10, len(week6_AME)))
...
class WeekButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        qs = random.sample(self.questions, min(15, len(self.questions)))  # currently 15; panel says 5 incoming
        for q in qs:                # Question text remains as-is
...
class AnswerButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        exam = is_exam_mode(interaction.user.id)
        ...
        # In Exam Mode: do NOT reveal correctness now
        if exam:
            try:
                await interaction.response.edit_message(
                    embed=discord.Embed(
                ...
                await interaction.followup.send("📝 Answer recorded. Next question…", ephemeral=True)
        else:
        ...
        await next_question(
            ...
            0 if self.idx == self.correct_idx else (self.parent_view.streak + 1)
        )
...
async def question_timer(interaction, q_index, score, questions, wrongs, user, streak):
    ...
    if active_timers.get(user.id) == q_index:
        ...
        # Neutral timeout message (doesn't reveal exam correctness beyond time)
        embed = discord.Embed(
            title="⏱ Time’s Up!",
            description=f"{user.mention} Q{q_index+1} recorded as no answer.",
            color=discord.Color.dark_grey()
        )
...
async def next_question(interaction, q_index, score, questions, wrongs, streak):
    ...
    # In Practice Mode: optional early end if 3 missed streak;
    # In Exam Mode: do NOT end early based on streak.
    exam = is_exam_mode(interaction.user.id)
    if (not exam) and streak >= 3:
    ...
    # Mode badge + small hint
    embed.set_footer(text=f"{mode_badge(interaction.user.id)}")
...
async def end_quiz(interaction, score, questions, wrongs, ended_by_streak=False):
    ...
    if ended_by_streak and (not is_exam_mode(interaction.user.id)):
    ...
    if questions and class_name in review_links:
        ...
        # ✅ split embed fields into 1024-character chunks
...
class ReviewButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        for wa in self.wrongs:
            ...
            # Truncate question title to 256 to be safe
            q_title = wa["q"] if len(wa["q"]) <= 256 else wa["q"][:253] + "..."
            embed.add_field(name=q_title, value=text_value, inline=False)
        await interaction.response.send_message(embed=embed, ephemeral=True)
